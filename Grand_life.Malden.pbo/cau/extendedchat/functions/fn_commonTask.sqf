#define THIS_FUNC FUNC(commonTask)#include "cau\extendedchat\functions\_macros.inc"#include "cau\extendedchat\functions\_defines.inc"
SWITCH_SYS_PARAMS;
switch _mode do {
case "ChannelColour":
    {
        ...
        case 0:
            {
                getArray(configFile >> "RscChatListDefault" >> "colorGlobalChannel")
            };
        case 1:
            {
                getArray(configFile >> "RscChatListDefault" >> "colorSideChannel")
            };
        case 2:
            {
                getArray(configFile >> "RscChatListDefault" >> "colorCommandChannel")
            };
        case 3:
            {
                getArray(configFile >> "RscChatListDefault" >> "colorGroupChannel")
            };
        case 4:
            {
                getArray(configFile >> "RscChatListDefault" >> "colorVehicleChannel")
            };
        case 5:
            {
                getArray(configFile >> "RscChatListDefault" >> "colorDirectChannel")
            };
        case 6;
        case 7;
        case 8;
        ...
        case 15:
            {
                ["get", [_params - 5, 0]] call FUNC(radioChannelCustom)
            };
        default {
                [[0.8, 0.1, 0.1, 1], [0, 0, 0, 0.4]] select _systemPrimary;
            };
        }] call THIS_FUNC;
    }; case "ChannelName":
    {
        ...
        case 15:
            {
                ["get", [_params - 5, 1]] call FUNC(radioChannelCustom)
            };
        case -2:
            {
                localize "STR_CAU_xChat_channel_command"
            };
        default {
                localize "STR_CAU_xChat_channel_system"
            };
        };
    };
case "SideName":
...
case "StreamSafeName":
    {
        ...
        [_name, localize "str_a3_rscdisplaydynamicgroups_you"] select(_uid == getPlayerUID player && {
...
case "ClientNamePrefix":
    {
        ...
        if (_channelID < 6) then {
            ...
            case 0:
                {
                    ["SideName", side group _unit] call THIS_FUNC
                };
            case 1;
            case 2:
                {
            ...
            count[["%CHANNEL_LABEL", _channelName], ["%UNIT_SIDE", ["SideName", playerSide] call FUNC(commonTask)], ["%UNIT_NAME", UNIT_NAME(player)], ["%UNIT_RANK", [player, "displayNameShort"] call BIS_fnc_rankParams], ["%UNIT_ID", ""], ["%UNIT_REF", ""], ["%UNIT_GRP_NAME", groupID group player], ["%UNIT_GRP_LEADER", ["", ", " + tolower localize "str_3den_attributes_triggeractivationowner_leader_text"] select(player == leader group player)], ["%UNIT_VEH_NAME", ["", getText(configFile >> "CfgVehicles" >> typeof player >> "displayName")] select(player != vehicle player)], ["%UNIT_VEH_POSITION ", mapGridPosition vehicle player]];
            _channelCallsign
        };
    };
...
case "formatSystemDate":
    {
        ...
        format["%1 %2, %3, %4:%5 %6", localize format["str_3den_attributes_date_month%1_text", _month], _day, _year, _hour, _minute, _meridiem];
    };
case "stringPrefix":
    {
...
case "stringTrimLeft":
    {
        ...
        private _split = _input splitString ""; {
            if ! (_x in _trim) exitWith {
                _input = _input select[_forEachIndex]
            };
        }
        forEach _split;
        _input
...
case "stringTrimRight":
    {
        ...
        reverse _split; {
            if ! (_x in _trim) exitWith {
                _input = _input select[0, count _input - _forEachIndex]
            };
        }
        forEach _split;
        _input
