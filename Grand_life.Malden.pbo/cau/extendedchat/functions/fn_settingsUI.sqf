#define THIS_FUNC FUNC(settingsUI)#define DISPLAY_NAME VAR(displaySettings)#include "cau\extendedchat\functions\_macros.inc"#include "cau\extendedchat\functions\_defines.inc"#include "cau\extendedchat\functions\_dikcodes.inc"#define DIALOG_SECTIONS 3#define DIALOG_SECTION_W 75#define DIALOG_W(DIALOG_SECTION_W * DIALOG_SECTIONS)#define DIALOG_H 105#define IDC_BUTTON_SAVE_SETTINGS 1#define IDC_EDIT_COMMAND_PREFIX 2#define IDC_COMBO_MAX_SAVED_LOGS 3#define IDC_COMBO_MAX_PRINTED_LOGS 4#define IDC_COMBO_PRINTED_LOG_TTL 5#define IDC_LABEL_AUTOCOMPLETE_KEYBIND 6#define IDC_LABEL_TOGGLE_CHAT_FEED_KEYBIND 7#define IDC_CB_HIDE_CHAT_FEED_ONLOAD_STREAMSAFE 8#define IDC_LABEL_TEXT_FONT 9#define IDC_COMBO_TEXT_FONT 10#define IDC_LABEL_TEXT_SIZE 11#define IDC_SLIDER_TEXT_SIZE 12#define IDC_LABEL_TEXT_COLOR 13#define IDC_LABEL_FEED_BACKGROUND_COLOR 14#define IDC_LABEL_TEXT_MENTION_COLOR 15#define IDC_LABEL_FEED_MENTION_BACKGROUND_COLOR 16#define IDC_CB_LOG_CONNECT 17#define IDC_CB_LOG_DISCONNECT 18#define IDC_CB_LOG_BATTLEYE_KICK 19#define IDC_CB_LOG_KILLED 20#define IDC_CB_CHANNEL_GLOBAL 21#define IDC_CB_CHANNEL_SIDE 22#define IDC_CB_CHANNEL_COMMAND 23#define IDC_CB_CHANNEL_GROUP 24#define IDC_CB_CHANNEL_VEHICLE 25#define IDC_CB_CHANNEL_DIRECT 26#define IDC_CB_CHANNEL_CUSTOM 27#define IDC_CB_LANGUAGE_FILTER 28#define IDC_BUTTON_LANGUAGE_FILTER_ADD 29#define IDC_BUTTON_LANGUAGE_FILTER_REM 30#define IDC_CB_WEBSITE_WHITELIST 31#define IDC_BUTTON_WEBSITE_WHITELIST_ADD 32#define IDC_BUTTON_WEBSITE_WHITELIST_REM 33#define IDC_LABEL_MUTED_PLAYERS 34#define IDC_BUTTON_MUTED_PLAYERS_ADD 35#define IDC_BUTTON_MUTED_PLAYERS_REM 36
...
case "init":
    {
        ...
        _display displayAddEventHandler["KeyDown", {
            ...
            if _active then {
                _ctrlLabel setVariable["setting", _key];
                _ctrlLabel ctrlSetText format[localize "STR_CAU_xChat_settings_configuration_toggle_chat_feed_keybind_label",
                if (_key == -1) then {
        ...
        count[["ctrlStatic", -1, [0, 0, 0, 0]], ["ctrlStaticBackgroundDisableTiles", -1, [getNumber(configFile >> "ctrlStaticBackgroundDisableTiles" >> "x"), getNumber(configFile >> "ctrlStaticBackgroundDisableTiles" >> "y"), getNumber(configFile >> "ctrlStaticBackgroundDisableTiles" >> "w"), getNumber(configFile >> "ctrlStaticBackgroundDisableTiles" >> "h")]], ["ctrlStaticBackground", -1, [PXCX(DIALOG_W), PXCY(DIALOG_H), PXW(DIALOG_W), PXH(DIALOG_H)]], ["ctrlStaticTitle", -1, [PXCX(DIALOG_W), PXCY(DIALOG_H), PXW(DIALOG_W), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "STR_CAU_xChat_settings_title";
        }], ["ctrlStaticFooter", -1, [PXCX(DIALOG_W), PXCY(DIALOG_H) + PXH(DIALOG_H) - PXH((SIZE_M + 2)), PXW(DIALOG_W), PXH((SIZE_M + 2))]], ["ctrlButtonClose", -1, [PXCX(DIALOG_W) + PXW(DIALOG_W) - PXW(((SIZE_M * 5) + 1)), PXCY(DIALOG_H) + PXH(DIALOG_H) - PXH((SIZE_M + 1)), PXW((SIZE_M * 5)), PXH(SIZE_M)], {
            _ctrl ctrlAddEventHandler["ButtonClick", {
                params["_ctrl"];
                USE_DISPLAY(ctrlParent _ctrl);
                ["closeDisplay", [_display]] call THIS_FUNC;
            }];
        }], ["ctrlButton", IDC_BUTTON_SAVE_SETTINGS, [PXCX(DIALOG_W) + PXW(DIALOG_W) - PXW(((SIZE_M * 5) * 2)) - PXW(2), PXCY(DIALOG_H) + PXH(DIALOG_H) - PXH((SIZE_M + 1)), PXW((SIZE_M * 5)), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "STR_CAU_xChat_settings_save_button";
            _ctrl ctrlEnable false;
            _ctrl ctrlAddEventHandler["ButtonClick", {
                ["ButtonSaveClicked"] call THIS_FUNC
            }];
        }], ["ctrlButton", -1, [PXCX(DIALOG_W) + PXW(1), PXCY(DIALOG_H) + PXH(DIALOG_H) - PXH((SIZE_M + 1)), PXW((SIZE_M * 5)), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "STR_CAU_xChat_settings_reset_button";
            _ctrl ctrlAddEventHandler["ButtonClick", {
                [[localize "STR_CAU_xChat_settings_reset_confirm"], localize "STR_CAU_xChat_settings_title", {
                    if _confirmed then {
                        ...
                        _parent spawn {
                            isNil {
                                ["init", _this] call THIS_FUNC
                            };
                        };
                    };
                },
                "Reset", "", THIS_DISPLAY] call CAU_UserInputMenus_fnc_guiMessage;
            }];
        }], ["ctrlStaticBackground", -1, [PXCX(DIALOG_W) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(DIALOG_H) - PXH(4) - PXH(SIZE_M) - PXH((SIZE_M + 2))], {
            _ctrl ctrlSetBackgroundColor[COLOR_OVERLAY_RGB, 0.2];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "STR_CAU_xChat_settings_configuration_title";
            _ctrl ctrlSetBackgroundColor[COLOR_OVERLAY_RGB, 0.4];
        }], ["ctrlStaticFrame", -1, [PXCX(DIALOG_W) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(DIALOG_H) - PXH(4) - PXH(SIZE_M) - PXH((SIZE_M + 2))]], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH(SIZE_M), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(SIZE_M)], {
        ...
        }], ["RscEdit", IDC_EDIT_COMMAND_PREFIX, [PXCX(DIALOG_W) + PXW(4), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(2) + PXH((SIZE_M * 2)), PXW((DIALOG_W / 3)) - PXW(6.5), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(3) + PXH((SIZE_M * 3)), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(SIZE_M)], {
        ...
        }], ["ctrlCombo", IDC_COMBO_MAX_SAVED_LOGS, [PXCX(DIALOG_W) + PXW(4), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(4) + PXH((SIZE_M * 4)), PXW((DIALOG_W / 3)) - PXW(6.5), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(5) + PXH((SIZE_M * 5)), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(SIZE_M)], {
        ...
        }], ["ctrlCombo", IDC_COMBO_MAX_PRINTED_LOGS, [PXCX(DIALOG_W) + PXW(4), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(6) + PXH((SIZE_M * 6)), PXW((DIALOG_W / 3)) - PXW(6.5), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(7) + PXH((SIZE_M * 7)), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(SIZE_M)], {
        ...
        }], ["ctrlCombo", IDC_COMBO_PRINTED_LOG_TTL, [PXCX(DIALOG_W) + PXW(4), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(8) + PXH((SIZE_M * 8)), PXW((DIALOG_W / 3)) - PXW(6.5), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", IDC_LABEL_AUTOCOMPLETE_KEYBIND, [PXCX(DIALOG_W) + PXW(2.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(10) + PXH((SIZE_M * 9)), PXW((DIALOG_W / 3)) - PXW(25), PXH(SIZE_M)], {
        ...
        }], ["ctrlButton", -1, [PXCX(DIALOG_W) + PXW(2) + (PXW((DIALOG_W / 3)) - PXW(2.5)) - PXW(22), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(10) + PXH((SIZE_M * 9)), PXW(20), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", IDC_LABEL_TOGGLE_CHAT_FEED_KEYBIND, [PXCX(DIALOG_W) + PXW(2.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(12) + PXH((SIZE_M * 10)), PXW((DIALOG_W / 3)) - PXW(25), PXH(SIZE_M)], {
            private _setting = ["get", VAL_SETTINGS_KEY_TOGGLE_CHAT_FEED_KEYBIND] call FUNC(settings);
            _ctrl setVariable["setting", _setting];
            _ctrl ctrlSetText format[localize "STR_CAU_xChat_settings_configuration_toggle_chat_feed_keybind_label",
            if (_setting == -1) then {
                localize "str_lib_info_na"
            } else {
                keyName _setting
        ...
        }], ["ctrlButton", -1, [PXCX(DIALOG_W) + PXW(2) + (PXW((DIALOG_W / 3)) - PXW(2.5)) - PXW(22), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(12) + PXH((SIZE_M * 10)), PXW(20), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "str_3den_display3den_menubar_edit_text";
            _ctrl ctrlAddEventHandler["ButtonClick", {
                params["_ctrl"];
                USE_DISPLAY(ctrlParent _ctrl);
        ...
        }], ["ctrlCheckbox", IDC_CB_HIDE_CHAT_FEED_ONLOAD_STREAMSAFE, [PXCX(DIALOG_W) + PXW(2.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(13) + PXH((SIZE_M * 11)), PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW(2.5) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(13) + PXH((SIZE_M * 11)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlStaticBackground", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(1.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(DIALOG_H) - PXH(4) - PXH(SIZE_M) - PXH((SIZE_M + 2))], {
            _ctrl ctrlSetBackgroundColor[COLOR_OVERLAY_RGB, 0.2];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(1.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "STR_CAU_xChat_settings_appearance_title";
            _ctrl ctrlSetBackgroundColor[COLOR_OVERLAY_RGB, 0.4];
        }], ["ctrlStaticFrame", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(1.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(DIALOG_H) - PXH(4) - PXH(SIZE_M) - PXH((SIZE_M + 2))]], ["ctrlStatic", IDC_LABEL_TEXT_FONT, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH(SIZE_M), PXW((DIALOG_W / 3)) - PXW(3), PXH(SIZE_M)], {
        ...
        }], ["ctrlCombo", IDC_COMBO_TEXT_FONT, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(4), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(2) + PXH((SIZE_M * 2)), PXW((DIALOG_W / 3)) - PXW(7), PXH(SIZE_M)], {
            private _setting = ["get", VAL_SETTINGS_KEY_TEXT_FONT] call FUNC(settings);
            private _fonts = ("true"
            configClasses(configFile >> "CfgFontFamilies")) apply {
                configName _x
            };
            _fonts sort true; {
                _ctrl lbAdd _x;
        ...
        }], ["ctrlStatic", IDC_LABEL_TEXT_SIZE, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(3) + PXH((SIZE_M * 3)), PXW((DIALOG_W / 3)) - PXW(3), PXH(SIZE_M)], {
        ...
        }], ["ctrlXSliderH", IDC_SLIDER_TEXT_SIZE, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(4), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(4) + PXH((SIZE_M * 4)), PXW((DIALOG_W / 3)) - PXW(7), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", IDC_LABEL_TEXT_COLOR, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(6) + PXH((SIZE_M * 5)), PXW((DIALOG_W / 3)) - PXW(25), PXH(SIZE_M)], {
            ...
            _ctrl setVariable["setting", _setting];
        }], ["ctrlButton", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2) + (PXW((DIALOG_W / 3)) - PXW(2.5)) - PXW(22.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(6) + PXH((SIZE_M * 5)), PXW(20), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "str_3den_display3den_menubar_edit_text";
            _ctrl ctrlAddEventHandler["ButtonClick", {
                params["_ctrl"];
                USE_DISPLAY(ctrlParent _ctrl);
                USE_CTRL(_ctrlLabel, IDC_LABEL_TEXT_COLOR);
                [[_ctrlLabel getVariable["setting", []]], "Message text color selection", {
                    if _confirmed then {
                        USE_DISPLAY(THIS_DISPLAY);
                        USE_CTRL(_ctrlLabel, IDC_LABEL_TEXT_COLOR);
                ...
                "", "", _display] call CAU_UserInputMenus_fnc_colorPicker;
            }];
        }], ["ctrlStatic", IDC_LABEL_FEED_BACKGROUND_COLOR, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(8) + PXH((SIZE_M * 6)), PXW((DIALOG_W / 3)) - PXW(25), PXH(SIZE_M)], {
            ...
            _ctrl setVariable["setting", _setting];
        }], ["ctrlButton", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2) + (PXW((DIALOG_W / 3)) - PXW(2.5)) - PXW(22.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(8) + PXH((SIZE_M * 6)), PXW(20), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "str_3den_display3den_menubar_edit_text";
            _ctrl ctrlAddEventHandler["ButtonClick", {
                params["_ctrl"];
                USE_DISPLAY(ctrlParent _ctrl);
                USE_CTRL(_ctrlLabel, IDC_LABEL_FEED_BACKGROUND_COLOR);
                [[_ctrlLabel getVariable["setting", []]], "Message background color selection", {
                    if _confirmed then {
                        USE_DISPLAY(THIS_DISPLAY);
                        USE_CTRL(_ctrlLabel, IDC_LABEL_FEED_BACKGROUND_COLOR);
        ...
        }], ["ctrlStatic", IDC_LABEL_TEXT_MENTION_COLOR, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(10) + PXH((SIZE_M * 7)), PXW((DIALOG_W / 3)) - PXW(25), PXH(SIZE_M)], {
            ...
            _ctrl ctrlSetTextColor _setting;
            _ctrl setVariable["setting", _setting];
        }], ["ctrlButton", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2) + (PXW((DIALOG_W / 3)) - PXW(2.5)) - PXW(22.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(10) + PXH((SIZE_M * 7)), PXW(20), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "str_3den_display3den_menubar_edit_text";
            _ctrl ctrlAddEventHandler["ButtonClick", {
                params["_ctrl"];
                USE_DISPLAY(ctrlParent _ctrl);
        ...
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(12) + PXH((SIZE_M * 8)), PXW((DIALOG_W / 3)) - PXW(25), PXH(SIZE_M)], {
            _ctrl ctrlSetBackgroundColor[0.1, 0.1, 0.1, 0.5];
        }], ["ctrlStatic", IDC_LABEL_FEED_MENTION_BACKGROUND_COLOR, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(12) + PXH((SIZE_M * 8)), PXW((DIALOG_W / 3)) - PXW(25), PXH(SIZE_M)], {
            ...
            _ctrl ctrlSetBackgroundColor _setting;
            _ctrl setVariable["setting", _setting];
        }], ["ctrlButton", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) + PXW(2) + (PXW((DIALOG_W / 3)) - PXW(2.5)) - PXW(22.5), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(12) + PXH((SIZE_M * 8)), PXW(20), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "str_3den_display3den_menubar_edit_text";
            _ctrl ctrlAddEventHandler["ButtonClick", {
                params["_ctrl"];
                USE_DISPLAY(ctrlParent _ctrl);
        ...
        }], ["ctrlStaticBackground", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(1), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(DIALOG_H) - PXH(4) - PXH(SIZE_M) - PXH((SIZE_M + 2))], {
            _ctrl ctrlSetBackgroundColor[COLOR_OVERLAY_RGB, 0.2];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(1), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(SIZE_M)], {
            _ctrl ctrlSetText localize "STR_CAU_xChat_settings_filter_title";
            _ctrl ctrlSetBackgroundColor[COLOR_OVERLAY_RGB, 0.4];
        }], ["ctrlStaticFrame", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(1), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2), PXW((DIALOG_W / 3)) - PXW(2.5), PXH(DIALOG_H) - PXH(4) - PXH(SIZE_M) - PXH((SIZE_M + 2))]], ["ctrlCheckbox", IDC_CB_LOG_CONNECT, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH(SIZE_M), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_CONNECTED] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH(SIZE_M), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlCheckbox", IDC_CB_LOG_DISCONNECT, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH((SIZE_M * 2)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_DISCONNECTED] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH((SIZE_M * 2)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlCheckbox", IDC_CB_LOG_BATTLEYE_KICK, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH((SIZE_M * 3)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_BATTLEYE_KICK] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH((SIZE_M * 3)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlCheckbox", IDC_CB_LOG_KILLED, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH((SIZE_M * 4)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_DEATH] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH(1) + PXH((SIZE_M * 4)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlCheckbox", IDC_CB_CHANNEL_GLOBAL, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 6)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_GLOBAL] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 6)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlCheckbox", IDC_CB_CHANNEL_SIDE, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 7)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_SIDE] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 7)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
            private _channelID = 1;
            private _channelName = ["ChannelName", _channelID] call FUNC(commonTask);
            _ctrl ctrlSetText _channelName;
            _ctrl ctrlSetTooltip format[localize "STR_CAU_xChat_settings_filter_channel_desc", _channelName];
            _ctrl ctrlSetTextColor(["ChannelColour", _channelID] call FUNC(commonTask));
        }], ["ctrlCheckbox", IDC_CB_CHANNEL_COMMAND, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 8)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_COMMAND] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 8)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
            private _channelID = 2;
            private _channelName = ["ChannelName", _channelID] call FUNC(commonTask);
            _ctrl ctrlSetText _channelName;
            _ctrl ctrlSetTooltip format[localize "STR_CAU_xChat_settings_filter_channel_desc", _channelName];
            _ctrl ctrlSetTextColor(["ChannelColour", _channelID] call FUNC(commonTask));
        }], ["ctrlCheckbox", IDC_CB_CHANNEL_GROUP, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 9)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_GROUP] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 9)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
            private _channelID = 3;
            private _channelName = ["ChannelName", _channelID] call FUNC(commonTask);
            _ctrl ctrlSetText _channelName;
            _ctrl ctrlSetTooltip format[localize "STR_CAU_xChat_settings_filter_channel_desc", _channelName];
            _ctrl ctrlSetTextColor(["ChannelColour", _channelID] call FUNC(commonTask));
        }], ["ctrlCheckbox", IDC_CB_CHANNEL_VEHICLE, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 10)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_VEHICLE] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 10)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
            private _channelID = 4;
            private _channelName = ["ChannelName", _channelID] call FUNC(commonTask);
            _ctrl ctrlSetText _channelName;
            _ctrl ctrlSetTooltip format[localize "STR_CAU_xChat_settings_filter_channel_desc", _channelName];
            _ctrl ctrlSetTextColor(["ChannelColour", _channelID] call FUNC(commonTask));
        }], ["ctrlCheckbox", IDC_CB_CHANNEL_DIRECT, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 11)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_DIRECT] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 11)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
            private _channelID = 5;
            private _channelName = ["ChannelName", _channelID] call FUNC(commonTask);
            _ctrl ctrlSetText _channelName;
            _ctrl ctrlSetTooltip format[localize "STR_CAU_xChat_settings_filter_channel_desc", _channelName];
            _ctrl ctrlSetTextColor(["ChannelColour", _channelID] call FUNC(commonTask));
        }], ["ctrlCheckbox", IDC_CB_CHANNEL_CUSTOM, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 12)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl cbSetChecked(["get", VAL_SETTINGS_KEY_PRINT_CUSTOM] call FUNC(settings));
            _ctrl ctrlAddEventHandler["CheckedChanged", {
                ["CBCheckedChanged"] call THIS_FUNC
            }];
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 12)), PXW((DIALOG_W / 3)) - PXW(4) - PXW(SIZE_M), PXH(SIZE_M)], {
            private _channelName = localize "STR_CAU_xChat_channels_custom";
            _ctrl ctrlSetText _channelName;
            _ctrl ctrlSetTooltip format[localize "STR_CAU_xChat_settings_filter_channel_desc", _channelName];
        }], ["ctrlCheckbox", IDC_CB_LANGUAGE_FILTER, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 14)), PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 14)), PXW((DIALOG_W / 3)) - PXW(5) - PXW(SIZE_M) * 3, PXH(SIZE_M)], {
        ...
        }], ["ctrlButtonPicture", IDC_BUTTON_LANGUAGE_FILTER_ADD, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW((DIALOG_W / 3)) - PXW(SIZE_M) * 3, PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 14)), PXW(SIZE_M), PXH(SIZE_M)], {
            ...
            _ctrl ctrlAddEventHandler["ButtonClick", {
                ...
                localize "str_single_create", "", _display] call CAU_UserInputMenus_fnc_text;
            }];
        }], ["ctrlButtonPicture", IDC_BUTTON_LANGUAGE_FILTER_REM, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW((DIALOG_W / 3)) - PXW(SIZE_M) * 2, PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(2) + PXH((SIZE_M * 14)), PXW(SIZE_M), PXH(SIZE_M)], {
            USE_CTRL(_ctrlCheckbox, IDC_CB_LANGUAGE_FILTER);
            _ctrl ctrlEnable cbChecked _ctrlCheckbox;
            ...
            _ctrl ctrlAddEventHandler["ButtonClick", {
                ...
                0, true], localize "STR_CAU_xChat_settings_filter_bad_language_remove_title", {
                    if _confirmed then {
                        USE_DISPLAY(THIS_DISPLAY);
                        USE_CTRL(_ctrlCheckbox, IDC_CB_LANGUAGE_FILTER);
                        _index sort false;
                ...
                localize "str_xbox_hint_remove", "", _display] call CAU_UserInputMenus_fnc_listBox;
            }];
        }], ["ctrlCheckbox", IDC_CB_WEBSITE_WHITELIST, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(3) + PXH((SIZE_M * 15)), PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlStatic", -1, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW(SIZE_M), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(3) + PXH((SIZE_M * 15)), PXW((DIALOG_W / 3)) - PXW(5) - PXW(SIZE_M) * 3, PXH(SIZE_M)], {
        ...
        }], ["ctrlButtonPicture", IDC_BUTTON_WEBSITE_WHITELIST_ADD, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW((DIALOG_W / 3)) - PXW(SIZE_M) * 3, PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(3) + PXH((SIZE_M * 15)), PXW(SIZE_M), PXH(SIZE_M)], {
        ...
        }], ["ctrlButtonPicture", IDC_BUTTON_WEBSITE_WHITELIST_REM, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW((DIALOG_W / 3)) - PXW(SIZE_M) * 2, PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(3) + PXH((SIZE_M * 15)), PXW(SIZE_M), PXH(SIZE_M)], {
            USE_CTRL(_ctrlCheckbox, IDC_CB_WEBSITE_WHITELIST);
            _ctrl ctrlEnable cbChecked _ctrlCheckbox;
            _ctrl ctrlSetText "\a3\3den\data\cfg3den\history\removefromlayer_ca.paa";
            ...
            _ctrl ctrlAddEventHandler["ButtonClick", {
                ...
                0, true], localize "STR_CAU_xChat_settings_filter_website_whitelist_remove_title", {
                    if _confirmed then {
                        USE_DISPLAY(THIS_DISPLAY);
                        USE_CTRL(_ctrlCheckbox, IDC_CB_WEBSITE_WHITELIST);
                        _index sort false;
        ...
        }], ["ctrlStatic", IDC_LABEL_MUTED_PLAYERS, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2), PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(4) + PXH((SIZE_M * 16)), PXW((DIALOG_W / 3)) - PXW(5) - PXW(SIZE_M) * 2, PXH(SIZE_M)], {
        ...
        }], ["ctrlButtonPicture", IDC_BUTTON_MUTED_PLAYERS_ADD, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW((DIALOG_W / 3)) - PXW(SIZE_M) * 3, PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(4) + PXH((SIZE_M * 16)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl ctrlSetText "\a3\3den\data\cfg3den\history\makenewlayer_ca.paa";
            ...
            _ctrl ctrlAddEventHandler["ButtonClick", {
                ...
                _terms = _terms apply {
                    _x #1};private _players= (allPlayers - [player]) select { ! (getPlayerUID _x in _terms)
                    }
                    apply {
                        [UNIT_NAME(_x), getPlayerUID _x]
                    };
                    _players sort true;
                    [[_players apply {
                        [[_x #0],nil,nil,nil,nil,str _x]},0,true],localize"STR_CAU_xChat_settings_filter_muted_players_add",{if _confirmed then{USE_DISPLAY(THIS_DISPLAY);USE_CTRL(_ctrlLabel,IDC_LABEL_MUTED_PLAYERS);private _terms= _ctrlLabel getVariable["setting", []]; _terms append(_data apply {
                            (parseSimpleArray _x) + [["formatSystemDate", systemTime] call FUNC(commonTask)]
                        });
                        ["ListModified"] call THIS_FUNC;
                    };
                },
                ...
                "",
                _display] call CAU_UserInputMenus_fnc_listBox;
            }];
        }], ["ctrlButtonPicture", IDC_BUTTON_MUTED_PLAYERS_REM, [PXCX(DIALOG_W) + PXW((DIALOG_W / 3)) * 2 + PXW(2) + PXW((DIALOG_W / 3)) - PXW(SIZE_M) * 2, PXCY(DIALOG_H) + PXH(SIZE_M) + PXH(4) + PXH((SIZE_M * 16)), PXW(SIZE_M), PXH(SIZE_M)], {
            _ctrl ctrlSetText "\a3\3den\data\cfg3den\history\removefromlayer_ca.paa";
            ...
            _ctrl ctrlAddEventHandler["ButtonClick", {
                ...
                _terms sort true;
                [[_terms apply {
                    [[_x #0],[_x# 2, [0.75, 0.75, 0.75, 1]], nil, nil, nil, _x #1]},0,true],localize"STR_CAU_xChat_settings_filter_muted_players_remove",{if _confirmed then{USE_DISPLAY(THIS_DISPLAY);USE_CTRL(_ctrlLabel,IDC_LABEL_MUTED_PLAYERS);_index sort false;private _terms= _ctrlLabel getVariable["setting", []]; {
                        _terms deleteAt _x
                    }
                    forEach _index;
                    ["ListModified"] call THIS_FUNC;
...
case "ButtonSaveClicked":
    {
        ...
        if (_commandPrefix == "" || {
            "<" in _commandPrefix || "&" in _commandPrefix || "&" in _commandPrefix || " " in _commandPrefix || _commandPrefix find ":" == 0
        }) then {
            _commandPrefix = nil
        };
        ["set", [VAL_SETTINGS_KEY_COMMAND_PREFIX,
        if (!isNil "_commandPrefix") then {
            _commandPrefix
        }]] call FUNC(settings);
        ...
        _languageTerms = _languageTerms apply {
            [count _x, _x]
        };
        _languageTerms sort false;
        ["set", [VAL_SETTINGS_KEY_BAD_LANGUAGE_FILTER_TERMS, _languageTerms apply {
            _x #1}]]call FUNC(settings);private _websiteWhitelist= _ctrlCBWebsiteWhitelist getVariable["setting", []];
            _websiteWhitelist = _websiteWhitelist apply {
                [count _x, _x]
            };
            _websiteWhitelist sort false;
            ["set", [VAL_SETTINGS_KEY_WEBSITE_WHITELIST_TERMS, _websiteWhitelist apply {
                _x #1}]]call FUNC(settings);saveProfileNamespace;systemChat format["Extended Chat: %1",localize"STR_CAU_xChat_settings_saved_alert"];true };case"closeDisplay":{_params params["_display"];USE_CTRL(_ctrlButton,IDC_BUTTON_SAVE_SETTINGS);if(ctrlEnabled _ctrlButton) then{[[localize"STR_CAU_xChat_settings_pending_changes"],localize"STR_CAU_xChat_settings_title",{if _confirmed then{USE_DISPLAY(THIS_DISPLAY);_display closeDisplay 2;};},"Discard","",_display ]call CAU_UserInputMenus_fnc_guiMessage;}else{_display closeDisplay 2 };};};
