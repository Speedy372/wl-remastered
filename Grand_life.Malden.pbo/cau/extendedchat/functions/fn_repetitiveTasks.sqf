#define THIS_FUNC FUNC(repetitiveTasks)#include "cau\extendedchat\functions\_macros.inc"#include "cau\extendedchat\functions\_defines.inc"#include "cau\extendedchat\functions\_dikcodes.inc"
if shownChat then {
    showChat false;
...
if (!isNull _display) then {
    ...
    if ! (_ctrlEdit getVariable[VAR_CHAT_INITIALIZED, false]) then {
        ...
        private _ctrlChatBG = (allControls _display) select(allControls _display findIf {
            ctrlClassName _x == "CA_Background"
        });
        ...
        private _eventUnscheduledLoop = {
            ...
            private _tasks = _display getVariable["tasks", []]; {
                _x params["_args", "_code", ["_execute", {
                    true
                }], ["_remove", {
                    true
                }]];
                if (_args call _execute) then {
                    _args call _code
                };
                if (_args call _remove) then {
                    _tasks set[_forEachIndex, 0]
                };
            }
            forEach _tasks;
        ...
        _ctrlCharCount ctrlSetFontHeight _ctrlEditPos #3;_ctrlCharCount ctrlSetBackgroundColor[profilenamespace getvariable["IGUI_BCG_RGB_R",0],profilenamespace getvariable["IGUI_BCG_RGB_G",1],profilenamespace getvariable["IGUI_BCG_RGB_B",1],profilenamespace getvariable["IGUI_BCG_RGB_A",0.8]];_ctrlCharCount ctrlSetPosition[_ctrlEditPos# 0,
        _ctrlEditPos #1-ctrlTextHeight _ctrlCharCount-PXH(.5),ctrlTextWidth _ctrlCharCount,ctrlTextHeight _ctrlCharCount+PXH(.5) ];_ctrlCharCount ctrlCommit 0;_displayOverlay setVariable["ctrlCharCount",_ctrlCharCount];["init",[_displayOverlay,_ctrlEdit]]call FUNC(suggestionUI);private _eventCharCountUpdate= {
            params["_ctrlEdit"];
            ...
            if (!isNull _ctrlCharCount) then {
                (_displayOverlay getVariable["tasks", []]) pushBack[[_ctrlEdit, _ctrlCharCount], {
                    ...
                    _ctrlCharCount ctrlSetTextColor[linearConversion[100, 150, _charCount, 1, [COLOR_NOTE_ERROR_RGB] #0,true],linearConversion[100,150,_charCount,1,[COLOR_NOTE_ERROR_RGB]# 1, true], linearConversion[100, 150, _charCount, 1, [COLOR_NOTE_ERROR_RGB] #2,true],1 ];_ctrlCharCount ctrlSetPositionW ctrlTextWidth _ctrlCharCount;_ctrlCharCount ctrlCommit 0;}];};};_ctrlEdit ctrlAddEventHandler["KeyDown",_eventCharCountUpdate];_ctrlCharCount setVariable["update",_eventCharCountUpdate];};};
# define VAR_CURATOR_INITIALIZED QUOTE(FUNC_SUBVAR(curatorInitialized))#define VAR_CURATOR_XPOS_UPDATE QUOTE(FUNC_SUBVAR(curatorXPosUpdate))
                    ...
                    if (isNull _display) then {
                        ...
                        VAR_MESSAGE_FEED_POS_X = VAR_MESSAGE_FEED_POS #0;VAR_NEW_MESSAGE_PENDING= true;
                        ...
                        VAR_MESSAGE_FEED_INSTANT_COMMIT = true;
                    };#define VAR_UPDATE_MESSAGES_TICK FUNC_SUBVAR(messagesUpdateTick)#define VAR_UPDATE_MESSAGES_CTRL_STATE QUOTE(FUNC_SUBVAR(ctrlState))#define VAR_UPDATE_MESSAGES_CTRL_TICK QUOTE(FUNC_SUBVAR(ctrlTick))
                    if (VAR_NEW_MESSAGE_PENDING || {
                        diag_tickTime >= (missionNameSpace getVariable[QUOTE(VAR_UPDATE_MESSAGES_TICK), 0])
                    }) then {
                        ...
                        for "_i"
                        from _activeMessageCtrls to 0 step - 1 do {
                            ...
                            _y = _y - _ctrlPosition #3-PXH(0.5);if((_ctrlPosition# 0) != _x ) then {
                                _ctrl ctrlSetPositionX _x
                            };
                            if ((_ctrlPosition #1)!= _y) then {
                    ...
                    };#define VAR_UPDATE_VON_TICK FUNC_SUBVAR(vonUpdateTick)#define VAR_UPDATE_VON_ISSPEAKING FUNC_SUBVAR(vonIsSpeaking)
                    if (VAR_ENABLE_VON_CTRL && {
                        diag_tickTime >= (missionNameSpace getVariable[QUOTE(VAR_UPDATE_VON_TICK), 0])
                    }) then {
                        ...
                        _ctrlVoipSpeakerPos set[2, (ctrlTextWidth _ctrlVoipSpeaker + PXW(0.1)) min(VAR_MESSAGE_FEED_POS #2)];_ctrlVoipSpeaker ctrlSetPosition _ctrlVoipSpeakerPos;_ctrlVoipSpeaker ctrlCommit 0;_ctrlVoipSpeakerPos set[3,ctrlTextHeight _ctrlVoipSpeaker];_ctrlVoipSpeaker ctrlSetPosition _ctrlVoipSpeakerPos;_ctrlVoipSpeaker ctrlCommit 0;_ctrlVoipSpeaker ctrlShow true;if!_playerIsSpeaking then{_playerIsSpeaking= !isNull findDisplay 55 && !isNull findDisplay 63;
                    ...
                    if ! (_isSpeaking isEqualTo _playerIsSpeaking) then {
                        ...
                        ["voice", [VAR_UPDATE_VON_ISSPEAKING, currentChannel, ["ClientNamePrefix", [player, currentChannel]] call FUNC(commonTask), getplayerUID player]] remoteExecCall[QUOTE(FUNC(log)), 2];
                    };
                };
                nil