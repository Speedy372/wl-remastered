#define THIS_FUNC FUNC(log)#include "cau\extendedchat\functions\_defines.inc"#define VAL_ENABLE_LOG_VAR(n) format["%1_%2", QUOTE(VAR_ENABLE_LOGGING), n]
...
case "toggle":
    {
        ...
        if ! (_params isEqualType 0) exitWith {
            false
        };
        ...
        _params params[["_channelID", -10, [0]], ["_text", "", [""]], ["_name", "", [""]], ["_pid", "", [""]]];
        ...
        private _log =
        if (_channelID < 0 || _channelID > 15) then {
            _text
        } else {
            format[(["(%3) ", ""] select(_pid == "")) + "%1: %2", _name, _text, _pid];
        };
        "ExtendedChat"
        callExtension[format["%1 %2", _channelID, _channel], [text _log]];
    };
case "voice":
    {
        if ! (missionNameSpace getVariable[QUOTE(VAR_ENABLE_LOGGING), false]) exitWith {};
        _params params[["_active", true, [true]], ["_channelID", -10, [0]], ["_name", "", [""]], ["_pid", "", [""]]];
        if ! (missionNameSpace getVariable[VAL_ENABLE_LOG_VAR(_channelID), true]) exitWith {};
        private _channel = ["ChannelName", _channelID] call FUNC(commonTask);
        if (_channel == "") exitWith {};
        private _log = format["(%1) %2 %4 speaking in %3", _pid, _name, _channel, ["stopped", "started"] select _active];
        "ExtendedChat"
        callExtension[format["%1 %2 VON", _channelID, _channel], [text _log]];
    };
}; nil 