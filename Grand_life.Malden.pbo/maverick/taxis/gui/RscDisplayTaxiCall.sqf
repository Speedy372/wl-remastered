...
case "onLoad":
    {
        ...
        _callTaxiButton ctrlAddEventHandler["ButtonClick", {
            ["callTaxiClicked", _this] call RscDisplayTaxiCall
        }];
        private _cancelTaxiButton = _display displayCtrl MAV_IDC_RSCDISPLAYTAXICALL_CANCELTAXI;
        _cancelTaxiButton ctrlAddEventHandler["ButtonClick", {
            ["cancelTaxiClicked", _this] call RscDisplayTaxiCall
        }];
        ...
        _map ctrlAddEventHandler["Draw", {
            ["mapDraw", _this] call RscDisplayTaxiCall
        }];
        _map ctrlAddEventHandler["MouseButtonDblClick", {
            ["mapDblClick", _this] call RscDisplayTaxiCall
        }];
        ...
        _params params[["_map", controlNull, [controlNull]]];
        private _display = ctrlParent _map;
        if (count mav_taxi_v_destination == 3) then {
            _map drawIcon["\a3\ui_f\data\Map\MapControl\waypoint_CA.paa", [1, 0, 0, 1], mav_taxi_v_destination, 30, 30, 0, "Destination", 1.5, 0.06, "RobotoCondensed"];
        };
    };
case "mapDblClick":
    {
        ...
        if (!isNull mav_taxi_v_myDriver) then {
            if ([5] call mav_taxi_fnc_preventClickSpam) exitWith {
                _exit = true;
            };
        } else {
...
case "callTaxiClicked":
    {
        if ([1] call mav_taxi_fnc_preventClickSpam) exitWith {
            [] call mav_taxi_fnc_hintClickSpam;
        };
        private _basePrice = getNumber(script_config >> "pricing" >> "basePrice");
        private _exit = false;
        ...
        if (mav_taxi_v_hasCalledTaxi) then {
            if (isNull mav_taxi_v_myDriver) then {
                ...
                [_display] spawn {
                    ...
                    private _ret = [["msgTaxiCancelConfirm"] call mav_taxi_fnc_getLocalization, ["msgTaxiCancelHeader"] call mav_taxi_fnc_getLocalization, ["msgTaxiCancelButtonConfirm"] call mav_taxi_fnc_getLocalization, ["msgTaxiCancelButtonCancel"] call mav_taxi_fnc_getLocalization] call BIS_fnc_guiMessage;
                    if (_ret) then {
                        mav_taxi_v_hasCalledTaxi = true;
...
case "showUI":
    {
        ...
        _lblAvailableDrivers ctrlSetText str count(playableUnits select {
            _x getVariable["mav_taxi_v_isOnDuty", false]
        });
        ...
        if (mav_taxi_v_hasCalledTaxi) then {
            ...
            if (isNull mav_taxi_v_myDriver) then {} else {
            ...
            _mainInfoText ctrlSetStructuredText parseText format[["msgMainInfoNoTaxiCalled"] call mav_taxi_fnc_getLocalization, count(AllPlayers select {
                _x getVariable["mav_taxi_v_isOnDuty", false]
            })];
