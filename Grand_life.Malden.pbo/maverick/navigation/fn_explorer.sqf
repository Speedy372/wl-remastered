scopeName "explorer";
params["_roadStart", "_roadEnd", "_preference"];
private _preset = 0;
...
from 0 to 1 step 0 do {
    ...
    if (_currentRoad isEqualTo _roadEnd) then {
        private _endNode = _closedSet select {
            (_x param[1]) isEqualTo _roadEnd
        };
        if (! (_endNode isEqualTo[])) then {
            ...
            if ((_endNode param[1]) isEqualType objNull) then {
                ...
                from 0 to 1 step 0 do {
                    ...
                    private _goBackNode = _closedSet select {
                        (_x param[1]) isEqualTo _cameFrom
                    };
    ...
    private _cost = -1; {
        ...
        if ((_roadCnt > 2) || {
            ...
            private _roadTypes = [[3.2, [1.5, 30]], [4.15, [1.25, 18]], [6.32, [1, 1]]];
        ...
        private _closedNode = _closedSet select {
            (_x param[1]) isEqualTo _neighbour
        };
