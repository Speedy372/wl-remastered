params[["_startPosition", [], [[]]], ["_endPosition", [], [[]]], ["_checkpoints", [], [[]]], ["_searchMode", "NORMAL", [""]], ["_stopExistingNavigation", true, [true]]];
...
if (_endPosition isEqualTo[] || {
    count _endPosition != 3
}) exitWith {
...
from 0 to 1 step 0 do {
    ...
    _previousPosition =
    if (!isNull(_totalRoute param[count _totalRoute - 1, objNull])) then {
        getPos(_totalRoute param[count _totalRoute - 1, objNull])
    } else {
        _startPosition
    };
    _targetPosition =
