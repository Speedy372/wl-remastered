#define MAVFNC MAV_guiNavigationNew#define MAVCFG missionConfigFile >> "maverick_navigation_cfg"#include "maverick\navigation\gui\gui_includes.h"
...
case "onLoad":
    {
        ...
        _display displayAddEventHandler["ChildDestroyed", {
            ["childDestroyed", _this] call MAVFNC
        }];
        ...
        _tabs ctrlAddEventHandler["ToolBoxSelChanged", {
            ["tabChanged", _this] call MAVFNC
        }];
        private _map = _display displayCtrl MAV_IDC_GUINAVIGATIONNEW_MAP;
        _map ctrlAddEventHandler["Draw", {
            ["mapDraw", _this] call MAVFNC
        }];
        _map ctrlAddEventHandler["MouseButtonDblClick", {
            ["mapDblClick", _this] call MAVFNC
        }];
        private _commonList = _display displayCtrl MAV_IDC_GUINAVIGATIONNEW_COMMON;
        _commonList ctrlAddEventHandler["LBSelChanged", {
            ["commonLocations", _this] call MAVFNC
        }];
        ...
        _savedList ctrlAddEventHandler["LBSelChanged", {
            ["savedLocation", _this] call MAVFNC
        }]; {
            private _nr = _x;
            private _ret = profileNamespace getVariable[format["MAV_gps_saved_%1_%2", _world, _nr], []];
            if (! (_ret isEqualTo[]) && {
                _ret isEqualTypeParams["", [], []]
            }) then {
        ...
        _newRoute ctrlAddEventHandler["ButtonClick", {
            ["newRoute", _this] call MAVFNC
        }];
        if (_showEndTab) then {
            _tabs lbSetCurSel 2;
...
case "commonLocations":
    {
        ...
        if (_roads isEqualTo[]) exitWith {
            hint(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgNoRoadsFound"));
        };
        _roads = _roads apply {
            [_x distance _location, _x]
        };
        _roads sort true;
        private _road = (_roads param[0]) param[1, objNull];
        if (isNull _road) exitWith {
            hint(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgInvalidRoadSegment"));
        };
        if ((roadsConnectedTo _road) isEqualTo[]) exitWith {
            hint(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgInvalidRoadSegment"));
        };
...
case "savedLocation":
    {
        ...
        (profileNamespace getVariable[format["MAV_gps_saved_%1_%2", toLower worldName, _ctrl lbValue _index], []]) params[["_title", "", [""]], ["_points", [], [[]]], ["_route", [], [[]]], ["_usedSearchType", "default", [""]]];
        ...
        private _routeBack = _route apply {
            (_x nearRoads 0.5) param[0, objNull]
        };
...
case "isValid":
    {
        ...
        (_display displayCtrl MAV_IDC_GUINAVIGATIONNEW_BTN) ctrlShow(!isNull _start && { ! isNull _destination
        } && {
            isNull _midpoint || {
                _valid
            }
        } && { ! (_start isEqualTo _destination)
        });
    };
case "newRoute":
    {
