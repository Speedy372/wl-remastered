#define MAVFNC MAV_guiNavigationSaved#define MAVCFG missionConfigFile >> "maverick_navigation_cfg"#include "maverick\navigation\gui\gui_includes.h"
...
case "start":
    {
        ...
        (profileNamespace getVariable[format["MAV_gps_saved_%1_%2", toLower worldName, _list lbValue _curSel], []]) params[["_title", "", [""]], ["_points", [], [[]]], ["_route", [], [[]]], ["_searchType", "default", [""]]];
        _route = _route apply {
            (_x nearRoads 0.5) param[0]
        };
        _route = _route select { ! isNull _x
        };
        _startRoad =
        if ! ((_points param[0]) isEqualTo[]) then {
            [_points param[0], 100, []] call BIS_fnc_nearestRoad
        } else {
            objNull
        };
        _middleRoad =
        if ! ((_points param[1]) isEqualTo[]) then {
            [_points param[1], 100, []] call BIS_fnc_nearestRoad
        } else {
            objNull
        };
        _endRoad =
        if ! ((_points param[2]) isEqualTo[]) then {
            [_points param[2], 100, []] call BIS_fnc_nearestRoad
        } else {
            objNull
        };
