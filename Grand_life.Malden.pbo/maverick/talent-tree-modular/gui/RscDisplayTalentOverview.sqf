scriptName "RscDisplayTalentOverview";#define __filename "RscDisplayTalentOverview.sqf"
disableSerialization;
try {
    params[["_mode", "", [""]], ["_parameters", [], [[]]], ["_display", displayNull, [displayNull]]];
    ...
    _updateGUI = {
        ...
        private _currentLevelProgress =
        if (_currentLevelRelativeExp != 0) then {
            ((life_currentExp - _currentLevelBottomExp) / (_currentLevelTopExp - _currentLevelBottomExp))
        } else {
            0
        };
    ...
    case "onLoad":
        {
            ...
            private _generateTreeView = {
                ...
                private _index = 0; {
                    ...
                    if (_first isEqualType[]) then {
                        ...
                        if ((str playerSide) in _limitedToFactions || {
                            count _limitedToFactions == 0
                        }) then {
            ...
            if (_requiredPerk != "") then {
                [life_currentExpPerks, getText(_selectedPerkConfig >> "requiredPerk")] call mav_ttm_fnc_hasPerk;
            } else {
                true
            };
            life_currentExpPerks call _iterate;
            if (((count _limitedToFactions) == 0) || {
                (str playerSide) in _limitedToFactions
            }) then {
