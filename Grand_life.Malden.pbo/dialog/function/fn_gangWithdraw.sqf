...
if (_ownerID != getPlayerUID player) exitWith {
    hint "Tylko lider gangu moze wyplacac pieniadze z konta gangu!";
};
if (isNil {
    (group player) getVariable "gang_name"
}) exitWith {
    hint localize "STR_ATM_NotInGang"
};
...
if (_value > _gFund) exitWith {
    hint localize "STR_ATM_NotEnoughFundsG"
};
if (_val < 100 && _gFund > 20000000) exitWith {
    hint localize "STR_ATM_WithdrawMin"
};
if ((group player getVariable["gbank_in_use_by", player]) != player) exitWith {
    hint localize "STR_ATM_WithdrawInUseG"
};
