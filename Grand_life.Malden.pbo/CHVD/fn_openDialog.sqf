[] spawn {
    ...
    forEach[[1900, CHVD_foot, CHVD_maxView, 1901, CHVD_footObj, CHVD_maxObj], [1902, CHVD_car, CHVD_maxView, 1903, CHVD_carObj, CHVD_maxObj], [1904, CHVD_air, CHVD_maxView, 1905, CHVD_airObj, CHVD_maxObj]]; {
        ...
        _textDisabled =
        if (isLocalized "STR_chvd_disabled") then {
        ...
        _textDynamic =
        if (isLocalized "STR_chvd_dynamic") then {
        ...
        _textFov =
        if (isLocalized "STR_chvd_fov") then {
        ...
        _handle = _ctrl ctrlSetEventHandler["LBSelChanged", format["[_this select 1, '%1',%2,%3,%4] call CHVD_fnc_onLBSelChanged_syncmode", _x select 1, _x select 2, _x select 3, _x select 4]];
    }
    forEach[[1404, "foot", 1410, 1901, 1007], [1406, "car", 1409, 1903, 1014], [1408, "air", 1411, 1905, 1018]]; {
        ...
        _handle = _ctrlDisplay ctrlSetEventHandler["keyDown", format["[_this select 0, '%1',%2,%3] call CHVD_fnc_onEBinput_syncmode", _x select 1, _x select 2, _x select 3]];
    }
    forEach[[1410, "foot", 1901, 1007], [1409, "car", 1903, 1014], [1411, "air", 1905, 1018]]; {
        _ctrl = ((findDisplay 2900) displayCtrl(_x select 0));
        if (CHVD_allowNoGrass) then {
            _textLow =
            if (isLocalized "STR_chvd_low") then {
        ...
        _textStandard =
        if (isLocalized "STR_chvd_standard") then {
        ...
        _textHigh =
        if (isLocalized "STR_chvd_high") then {
        ...
        _textVeryHigh =
        if (isLocalized "STR_chvd_veryHigh") then {
        ...
        _textUltra =
        if (isLocalized "STR_chvd_ultra") then {
