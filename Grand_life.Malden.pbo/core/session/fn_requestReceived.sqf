...
case west:
    {
        ...
        CONST(life_donator, (_this select 12));
...
case civilian:
    {
        ...
        CONST(life_medicLevel, 0);
        CONST(life_donator, (_this select 13));
        ...
        forEach life_houses;
        life_gangData = [_this select 14, _this select 15];
...
case independent:
    {
        ...
        CONST(life_coplevel, 0);
        CONST(life_donator, (_this select 11));
        if (LIFE_SETTINGS(getNumber, "save_playerStats") isEqualTo 1) then {
            life_hunger = ((_this select 9) select 0);
            life_thirst = ((_this select 9) select 1);
            player setDamage((_this select 9) select 2);
...
BTC = missionNamespace getVariable[format["bitkoiny_%1", getPlayerUID player], 0];
missionnamespace setVariable[format["bitkoiny_%1", getPlayerUID player], nil, true];
