...
if (isNull Life_trunk_vehicle OR ! alive Life_trunk_vehicle) exitWith {
    [localize "STR_MISC_VehDoesntExist", true, "fast", "orange", "Error"] spawn Life_fnc_msg;
};
if (!alive player) exitwith {
    closeDialog 0;
};
if ((life_trunk_vehicle getVariable["trunk_in_use_by", player]) != player) exitWith {
    closeDialog 0;
    hint localize "STR_MISC_VehInvUse";
};
_nearUnits = (nearestObjects[player, ["CAManBase"], 8]) arrayIntersect playableUnits;
if (count _nearUnits > 1) exitWith {
    hint localize "STR_NOTF_PlayerNear"
};
if ((lbCurSel 3502) == -1) exitWith {
    [localize "STR_Global_NoSelection", true, "fast", "orange", "Error"] spawn Life_fnc_msg;
};
...
_num =
if (_num1 <= ((((Life_trunk_vehicle getVariable "Trunk") select 0) select(lbCurSel 3502)) select 1)) then {
    _num1
} else {
    ((((Life_trunk_vehicle getVariable "Trunk") select 0) select(lbCurSel 3502)) select 1)
};
if (_num < 1) exitWith {
    ["Du hast kein Platz dafür.", true, "fast", "orange", "Error"] spawn Life_fnc_msg;
};
...
if (_num > _value) exitWith {
    [localize "STR_MISC_NotEnough", true, "fast", "orange", "Error"] spawn Life_fnc_msg;
};
...
if (_ctrl == "money") then {
    ...
    gl_money_cash = gl_money_cash + _num;
...
_log = format["%1, %2 wzial %3 z bagaznika w ilosci %4", name player, getPlayerUID player, _ctrl, [_num] call life_fnc_numberText];
