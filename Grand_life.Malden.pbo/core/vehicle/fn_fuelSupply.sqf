...
closeDialog 0;
if (_vehicle getVariable["fuelTankWork", false]) exitWith {
    titleText[localize "STR_FuelTank_InUse", "PLAIN"]
};
if ! (local _vehicle) exitWith {
    titleText[localize "STR_MISC_VehLocal", "PLAIN"]
};
...
_vehTank params[["_fuelSpace", 19500, [0]], ["_fuelState", 0, [0]]];
if (_fuelState isEqualTo 0) exitWith {
    titleText[localize "STR_FuelTank_Empty", "PLAIN"];
    life_action_inUse = false;
};
private _trucks = (nearestObjects[_vehicle, ["C_Van_01_fuel_F", "I_Truck_02_fuel_F", "B_Truck_01_fuel_F"], 100]) findIf {
    _x getVariable["fuelTankWork", false]
};
if ! (_trucks isEqualTo - 1) exitWith {
    titleText[localize "STR_FuelTank_AnotherInUse", "PLAIN"];
    life_action_inUse = false
};
...
if (_fuelFeedState isEqualTo 0) exitWith {
    titleText[localize "STR_FuelTank_FeedFull", "PLAIN"];
    life_action_inUse = false;
};
private _shortest = 100000; {
    private _distance = _vehicle distance(getMarkerPos _x);
    if (_distance < _shortest) then {
        _shortest = _distance
    };
}
forEach["fuel_storage_1", "fuel_storage_2"];
if (_shortest < 1000) exitWith {
    titleText[localize "STR_FuelTank_PipeLine", "PLAIN"];
    life_action_inUse = false;
};
...
waitUntil {
    ...
    if (isEngineOn _vehicle) exitWith {
        titleText[localize "STR_FuelTank_Stopped", "PLAIN"];
        true
    };
    if ! (_vehicle getVariable["fuelTankWork", false]) exitWith {
        titleText[localize "STR_FuelTank_Stopped", "PLAIN"];
    ...
    if (player distance _vehicle > 20) exitWith {
        titleText[localize "STR_FuelTank_Stopped", "PLAIN"];
        true
    };
    if ! (local _vehicle) exitWith {
    ...
    if (_fuelState isEqualTo 0 || _fuelFeedState <= 0) exitWith {
        true
    };
    sleep 1;
    false
