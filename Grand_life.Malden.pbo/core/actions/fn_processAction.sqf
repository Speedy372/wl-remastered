...
if (_vendor in [coke_processor, heroin_processor]) then {
    ...
    if (_cops <= 3) exitWith {
        ["Przetwarzanie na kryjówce gangu jest mozliwe od 3 policjantow online!", false] spawn domsg;
    };
} else {
...
if (_hasLicense) then {
    ...
    if (_minimumConversions isEqualTo(_totalConversions call BIS_fnc_lowestNum)) then {
        ["Twoje surowce zostały przetworzone!", "", 5] call pedro_fnc_pow;
    } else {
        hint localize "STR_Process_Partial";
    };
    life_is_processing = false;
    life_action_inUse = false;
    _log = format["%1, %2 przetworzyl %3 na %4", name player, getPlayerUID player, _oldItem, _newItem];
    [_log, "przetworz"] remoteExecCall["A3Log", 2];
} else {
