...
if (! (playerSide isEqualTo west) && independent countSide playableUnits > 0) exitWith {
    ["Możesz przeprowadzać RKO tylko , gdy medyków nie ma na służbie.", false] spawn domsg;
};
if ((playerSide isEqualTo west) && independent countSide playableUnits > 0) exitWith {
    ["Możesz przeprowadzać RKO tylko , gdy medyków nie ma na służbie.", false] spawn domsg;
};
...
_log = format["%1, %2 podniosl(CPR) %3 %4", name player, getPlayerUID player, _targetName, getPlayerUID _target];
