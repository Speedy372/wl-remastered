...
[] spawn {
    ...
    do {
        ...
        waitUntil {
            uiSleep 1;
            (!(player getVariable["tied", false]) || (time - _time) > (20 * 60))
        };
        ...
        if (! ([west, getPos player, 30] call life_fnc_nearUnits) && (player getVariable["tied", false]) && isNull objectParent player) exitWith {
            ...
            detach player;
            hintSilent "Udało ci się uwolnić."
        };
    };
};
hint format["%1 cię związał.", name _civ];
...
[] spawn {
    ...
    do {
        ...
        waitUntil {
            animationState player != "AmovPercMstpSnonWnonDnon_Ease" || !(player getvariable["tied", false]) || vehicle player != _state
        };
