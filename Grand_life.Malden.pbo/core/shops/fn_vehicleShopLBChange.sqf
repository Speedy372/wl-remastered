...
if ((_classNameShop == "b_market_car") || (_classNameShop == "b_market_heli")) then {
    ...
    _rentMultiplier = 1;
    ctrlShow[2330, true];
    (CONTROL(2300, 2303)) ctrlSetStructuredText parseText format[(localize "STR_Shop_Veh_UI_Rental") + " <t color='#8cff9b'>%1 BTC</t><br/>" + (localize "STR_Shop_Veh_UI_Ownership") + " <t color='#8cff9b'>%2 BTC</t><br/>" + (localize "STR_Shop_Veh_UI_MaxSpeed") + " %3 km/h<br/>" + (localize "STR_Shop_Veh_UI_HPower") + " %4<br/>" + (localize "STR_Shop_Veh_UI_PSeats") + " %5<br/>" + (localize "STR_Shop_Veh_UI_Trunk") + " %6<br/>" + (localize "STR_Shop_Veh_UI_Fuel") + " %7<br/>" + (localize "STR_Shop_Veh_UI_Armor") + " %8", [round(_initalPrice * _rentMultiplier)] call life_fnc_numberText, [round(_initalPrice * _buyMultiplier)] call life_fnc_numberText, _maxspeed, _horsepower, _passengerseats,
...
for "_i"
from 0 to(count(_colorArray) - 1) do {
