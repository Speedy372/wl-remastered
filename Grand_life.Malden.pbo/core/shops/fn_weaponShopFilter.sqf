...
lbClear _itemList;
private "_config";
if ((uiNamespace getVariable["Weapon_Magazine", 0]) isEqualTo 1 || {
    (uiNamespace getVariable["Weapon_Accessories", 0]) isEqualTo 1
}) then {
    if ((uiNamespace getVariable["Weapon_Magazine", 0]) isEqualTo 1) then {
        _config = M_CONFIG(getArray, "WeaponShops", _shop, "mags"); {
            ...
            if (_bool) then {
                ...
                count(uiNamespace getVariable["Magazine_Array", []]);
                if (_count > 0) then {
        ...
        _config = M_CONFIG(getArray, "WeaponShops", _shop, "accs"); {
            ...
            if (_bool) then {
                ...
                count(uiNamespace getVariable["Accessories_Array", []]);
                if (_count > 0) then {
                    _itemInfo = [(_x select 0)] call life_fnc_fetchCfgDetails;
                    _itemList lbAdd format["%1",
                    if (! ((_x select 1) isEqualTo "")) then {
