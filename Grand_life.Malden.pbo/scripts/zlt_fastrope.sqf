#define MAX_SPEED_WHILE_FASTROPING 15#define MAX_SPEED_ROPES_AVAIL 25#define STR_TOSS_ROPES "Wyrzuć Liny"#define STR_FAST_ROPE "Szybka lina (spacja)"#define STR_CUT_ROPES "Utnij Liny"
...
zlt_rope_helidata = [[["O_Heli_Light_02_unarmed_F", "O_Heli_Light_02_F"], [1.35, 1.35, -24.95], [-1.45, 1.35, -24.95]], [["B_Heli_Transport_01_F", "B_Heli_Transport_01_camo_F"], [-1.11, 2.5, -24.7], [1.11, 2.5, -24.7]], [["O_Heli_Attack_02_F", "O_Heli_Attack_02_black_F"], [1.3, 1.3, -25], [-1.3, 1.3, -25]], [["I_Heli_Transport_02_F"], [0, -5, -26], []], [["B_Heli_Light_01_F"], [0.6, 0.5, -25.9], [-0.8, 0.5, -25.9]]];
zlt_fnc_tossropes = {
    ...
    _heli spawn {
        ...
        _heli = _this;
        while {
            alive _heli and count(_heli getvariable["zlt_ropes", []]) != 0 and abs(speed _heli) < MAX_SPEED_ROPES_AVAIL
        }
        do {
            sleep 0.3;
...
zlt_fnc_ropes_cond = {
    _veh = vehicle player;
    _flag = (player == driver _veh) and(_veh != player) and {
        (not zlt_mutexAction)
    }
    and {
    ...
    and {
        alive player and alive _veh and(abs(speed _veh) < MAX_SPEED_ROPES_AVAIL)
    };
    _flag;
};
...
zlt_fnc_fastropeUnit = {
    ...
    while {
        alive _unit and(getpos _unit select 2) > 1 and(abs(speed _heli)) < MAX_SPEED_WHILE_FASTROPING and _zc > -24
    }
    do {
