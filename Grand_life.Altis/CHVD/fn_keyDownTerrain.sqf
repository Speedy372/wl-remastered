...
_terrainString = "";
for "_i"
from(37.125) to 3.125 step - 1 do {
    if (round((sqrt _terrainGrid) * 10) - 18 >= 53 / 37.125 * _i) then {
        _terrainString = _terrainString + "·";
    } else {
        _terrainString = _terrainString + "I";
    };
}; _terrainQualityArray = [["Low", localize "STR_chvd_low"] select(isLocalized "STR_chvd_low"), ["Standart", localize "STR_chvd_standard"] select(isLocalized "STR_chvd_standard"), ["High", localize "STR_chvd_high"] select(isLocalized "STR_chvd_high"), ["Very High", localize "STR_chvd_veryHigh"] select(isLocalized "STR_chvd_veryHigh"), ["Ultra", localize "STR_chvd_ultra"] select(isLocalized "STR_chvd_ultra")];
_terrainQuality = _terrainQualityArray select _terrainIndex;
_textTerrainQuality =
if (isLocalized "STR_chvd_terrainQuality") then {
    localize "STR_chvd_terrainQuality"
} else {
    "Terrain Quality"
};
_textTerrainGrid =
if (isLocalized "STR_chvd_terrainGrid") then {
...
</t>", [profilenamespace getvariable['GUI_BCG_RGB_R', 0.3843], profilenamespace getvariable['GUI_BCG_RGB_G', 0.7019], profilenamespace getvariable['GUI_BCG_RGB_B', 0.8862], profilenamespace getvariable['GUI_BCG_RGB_A', 0.7]] call BIS_fnc_colorRGBAtoHTML, _textTerrainQuality, _terrainQuality, _textTerrainGrid, _terrainGrid, _terrainString];
