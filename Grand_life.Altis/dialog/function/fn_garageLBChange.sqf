...
if (! (_insurancePrice isEqualType 0) || _insurancePrice < 1) then {
    _insurancePrice = 500;
};
...
Cena ubezpieczenia: <t color='#8cff9b'>$%9</t><br/>
Ubezpieczenie: %10 <br/>
...
", [_retrievePrice] call life_fnc_numberText, [_sellPrice] call life_fnc_numberText, (_vehicleInfo select 8), (_vehicleInfo select 11), (_vehicleInfo select 10),
if (_trunkSpace isEqualTo - 1) then {
    "None"
} else {
    _trunkSpace
},
(_vehicleInfo select 12), _vehicleColor, [_insurancePrice] call life_fnc_numberText,
if (_insurance == 1) then {
    "<t color='#8cff9b'>Ubezpieczony</t>"
} else {
    "<t color='#FF0000'>Nieubezpieczony</t>"
},
