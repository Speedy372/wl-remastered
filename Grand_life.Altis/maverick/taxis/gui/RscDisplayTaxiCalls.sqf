...
case "onLoad":
    {
        ...
        _btnGoOnDuty ctrlAddEventHandler["ButtonClick", {
            ["btnOnDuty", _this] call RscDisplayTaxiCalls
        }];
        private _btnGoOffDuty = _display displayCtrl MAV_IDC_RSCDISPLAYTAXICALLS_GOOFFDUTYBTN;
        _btnGoOffDuty ctrlAddEventHandler["ButtonClick", {
            ["btnOffDuty", _this] call RscDisplayTaxiCalls
        }];
        private _btnAcceptCall = _display displayCtrl MAV_IDC_RSCDISPLAYTAXICALLS_ACCEPTCALL;
        _btnAcceptCall ctrlAddEventHandler["ButtonClick", {
            ["btnAccept", _this] call RscDisplayTaxiCalls
        }];
        private _btnCancelFare = _display displayCtrl MAV_IDC_RSCDISPLAYTAXICALLS_CANCELCALL;
        _btnCancelFare ctrlAddEventHandler["ButtonClick", {
            ["btnCancel", _this] call RscDisplayTaxiCalls
        }];
        private _listnbox = _display displayCtrl MAV_IDC_RSCDISPLAYTAXICALLS_LISTNBOX;
        _listnbox ctrlAddEventHandler["LBSelChanged", {
            ["lbSelectionChanged", _this] call RscDisplayTaxiCalls
        }];
        private _btnRefresh = _display displayCtrl MAV_IDC_RSCDISPLAYTAXICALLS_REFRESHBTN;
        _btnRefresh ctrlAddEventHandler["ButtonClick", {
            ["refreshButtonClicked", _this] call RscDisplayTaxiCalls
        }];
...
case "refreshButtonClicked":
    {
        _params params[["_buttonCtrl", controlNull, [controlNull]]];
        ...
        _taxiCallsListbox lnbSetColor[[0, 4], [0.99, 0.49, 0, 1]]; {
            ...
            _index = _taxiCallsListbox lnbAddRow[name _unit, _distanceText, [serverTime - _createdAt, "MM:SS"] call BIS_fnc_secondsToString, [getPos _unit] call mav_taxi_fnc_nearestLocation, [_destinationPosition] call mav_taxi_fnc_nearestLocation];
...
case "lbSelectionChanged":
    {
        _params params[["_ctrl", controlNull, [controlNull]], ["_index", -1, [-1]]];
...
case "btnOnDuty":
    {
        if ([] call mav_taxi_fnc_preventClickSpam) exitWith {
            [] call mav_taxi_fnc_hintClickSpam;
        };
        _params params[["_buttonCtrl", controlNull, [controlNull]]];
        private _display = ctrlParent _buttonCtrl;
...
case "btnCancel":
    {
        ...
        [_display] spawn {
            ...
            private _ret = [["msgTaxiCancelDriverConfirm"] call mav_taxi_fnc_getLocalization, ["msgTaxiCancelHeader"] call mav_taxi_fnc_getLocalization, ["msgTaxiCancelButtonConfirm"] call mav_taxi_fnc_getLocalization, ["msgTaxiCancelButtonCancel"] call mav_taxi_fnc_getLocalization] call BIS_fnc_guiMessage;
            if (_ret) then {
