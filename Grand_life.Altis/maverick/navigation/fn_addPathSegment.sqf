...
params[["_startRoad", objNull, [objNull]], ["_endRoad", objNull, [objNull]]];
if (isNull _startRoad || {
    isNull _endRoad
}) exitWith {
    false
};
...
private _centerPosition = [((_startRoadPosition select 0) + (_endRoadPosition select 0)) / 2, ((_startRoadPosition select 1) + (_endRoadPosition select 1)) / 2, ((_startRoadPosition select 2) + (_endRoadPosition select 2)) / 2];
...
while { ! ((allMapMarkers find _markerName) isEqualTo - 1)
}
do {
