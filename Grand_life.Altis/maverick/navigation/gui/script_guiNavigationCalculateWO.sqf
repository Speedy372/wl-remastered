#define MAVFNC MAV_guiNavigationCalculateWO#define MAVCFG missionConfigFile >> "maverick_navigation_cfg"#include "maverick\navigation\gui\gui_includes.h"
...
case "onLoad":
    {
        ...
        _tabControl ctrlAddEventHandler["ToolBoxSelChanged", {
        ...
        if ! (MAV_guiNavigationCalculate_Route isEqualTo[]) then {
            ...
            private _searchTypeString =
            if (mav_gps_v_currentRouteSearchType == "highway") then {
                "Highway"
            } else {
                "Default"
            };
...
case "script":
    {
        _params params[["_display", displayNull, [displayNull]], ["_start", objNull, [objNull]], ["_wayPoints", [], [[]]], ["_option", -1, [0]]];
...
case "tabChanged":
    {
        ...
        if (_index == 0) then {
            ...
            forEach[MAV_IDC_GUINAVIGATIONCALCULATEWO_TXT]; {
                (_display displayCtrl _x) ctrlShow false;
            }
            forEach[MAV_IDC_GUINAVIGATIONCALCULATEWO_MAP];
        } else {
            {
                (_display displayCtrl _x) ctrlShow false;
            }
