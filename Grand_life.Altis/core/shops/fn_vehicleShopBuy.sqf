...
if ((life_veh_shop select 0) == "med_air_hs") then {
    if (nearestObjects[(getMarkerPos _spawnPoints), ["Air"], 35] isEqualTo[]) exitWith {
    ...
    if ((life_veh_shop select 0) == "cop_air") then {
        ...
        if (_spawnPoints isEqualType[]) then {
            {
                if ((nearestObjects[(getMarkerPos _x), ["Car", "Ship", "Air"], 5]) isEqualTo[]) exitWith {
                    _spawnPoint = _x
                };
                true
            }
            ...
            if (nearestObjects[(getMarkerPos _spawnPoints), ["Car", "Ship", "Air"], 5] isEqualTo[]) exitWith {
                _spawnPoint = _spawnPoints
            };
        };
    };
...
if ((life_veh_shop select 0) == "med_air_hs") then {
    ...
    if (_spawnPoint find "edic_spawn_" > -1) then {
        _hs = nearestObjects[getMarkerPos _spawnPoint, ["Land_Hospital_side2_F"], 50] select 0;
        _vehicle setPosATL(_hs modelToWorld[-0.4, -4, 12.65]);
    } else {
        _vehicle setPos[(getMarkerPos _spawnPoint)#0, (getMarkerPos _spawnPoint)#1, 1.2];
    };
    sleep 0.6;
} else {
    if ((life_veh_shop select 0) == "cop_air") then {
...
_vehicle setPlateNumber "Grand-Life";
