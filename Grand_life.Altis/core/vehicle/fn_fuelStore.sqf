...
(_vehicle getVariable["fuelTank", []]) params[["_fuelSpace", (getNumber(missionConfigFile >> "LifeCfgVehicles" >> (typeOf _vehicle) >> "vFuelSpace")), [0]], ["_fuelState", 0, [0]]];
...
waitUntil {
    ...
    _progress progressSetPosition _fuelLevel;
    _pgText ctrlSetText format["Tank  %1 Ltr / %2 Ltr", _fuelState, _fuelSpace];
    if (_fuelState >= _fuelSpace) exitWith {
        titleText[localize "STR_FuelTank_Full", "PLAIN"];
        true
    };
    sleep 0.5;
    false
