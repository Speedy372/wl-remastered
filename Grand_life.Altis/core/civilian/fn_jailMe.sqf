...
if (_time <= 0) then {
    _time = time + (15 * 60);
    hintC "Proszę zgłosić do administracji JAIL_FALLBACK_15, czas jest zerowy!";
};
[_bad, _time] spawn {
...
do {
    if ((round(_time - time)) > 0) then {
        ...
        if (round(_time - time) > 60) then {
            format["%1 Minut", round(round(_time - time) / 60)]
        } else {
            format["%1 Sekund", round(_time - time)]
        };
        if (isNil "life_canpay_bail") then {
            hintSilent format["Pozostały czas:\n %1\n\nCzy możesz zapłacić kaucję: %3\nWysokość kaucji: $%2", _countDown, [life_bail_amount] call life_fnc_numberText, "dass"];
        } else {
            hintSilent format["Pozostały czas:\n %1\n", _countDown];
        };
    };
