...
if (! (_resource in ["uran_niep_med", "dust", "apple", "pankuronium", "algi", "cannabis", "cukier"])) exitWith {
    life_action_inUse = false;
};
if (_requiredItem != "") then {
    _valItem = missionNamespace getVariable "life_inv_" + _requiredItem;
    if (_valItem < 1) exitWith {
        switch (_requiredItem) do {}; life_action_inUse = false;
        _exit = true;
    };
};
if (_exit) exitWith {
...
case "pickaxe":
    {
        ...
        [player, "pixie", 35, 1] remoteExecCall["life_fnc_say3D", RCLIENT]
    };
}; for "_i"
from 0 to 4 do {
