...
if (count crew _vehicle isEqualTo 0) then {
    ...
    [_vehicle] remoteExec["TON_fnc_vehicleDead", RSERV];
    if (playerSide isEqualTo west) then {
        ...
        if (_vehicle in life_vehicles) then {
            ...
            BANK = BANK - _value;
            ["odcholowanie"] spawn mav_ttm_fnc_addExp;
        } else {
            hint format[localize "STR_NOTF_Crushed", [_value] call life_fnc_numberText, _type];
            BANK = BANK + _value;
            ["odcholowanie"] spawn mav_ttm_fnc_addExp;
...
life_action_inUse = false;
_log = format["%1, %2 skonfiskowal %3", name player, getPlayerUID player, _type];
[_log, "konfpoj"] remoteExecCall["A3Log", 2];