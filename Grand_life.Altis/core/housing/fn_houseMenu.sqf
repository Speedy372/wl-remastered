...
if (_curTarget isKindOf "House_F" && playerSide isEqualTo west) exitWith {
    ...
    private _altisArray = [16106.44, 16865.24, 0.097];
    private _tanoaArray = [16106.44, 16865.24, 0.097];
    private _pos = [[["Altis", _altisArray], ["Tanoa", _tanoaArray]]] call TON_fnc_terrainSort;
    if ((nearestObject[_pos, "Land_Dome_Small_F"]) isEqualTo _curTarget || (nearestObject[_pos, _vaultHouse]) isEqualTo _curTarget) then {
...
if (! (_curTarget in life_vehicles) || isNil {
    ...
    private _buildingPurchaseString = ["STR_pInAct_BuyGarage", "STR_pInAct_BuyHouse"] select _isHouse;
...
if (! (_var isEqualTo - 1) && {
    ...
    _var isEqualTo((group player) getVariable["gang_hq_id", -1])
}) then {
