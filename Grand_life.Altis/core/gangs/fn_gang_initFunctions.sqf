...
life_fnc_gang_toperm = compileFinal '
    ...
    if(!_result) exitWith {};
    if(_price > gl_bank) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
    gl_bank = gl_bank - _price;
...
life_fnc_gang_bankupdate = compileFinal '
    ...
    ctrlSetText[220124,[gl_bank] call life_fnc_numberText];
';
