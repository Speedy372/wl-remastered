...
if ! (playerSide isEqualTo west) exitWith {
    hint localize "STR_Cop_wantedList_notCop";
};
if ((lbCurSel 2406) isEqualTo - 1) exitWith {
    hint localize "STR_Cop_wantedList_noPlayerSelected";
};
if ((lbCurSel 2407) isEqualTo - 1) exitWith {
    hint localize "STR_Cop_wantedList_noCrimeSelected";
};
...
_log = format["%1, %2 dodal %3 %4 do listy za %5", name player, getPlayerUID player, getPlayerUID _unit, name _unit, [(parseNumber(_amount))] call life_fnc_numberText];
[_log, "listaposz"] remoteExecCall["A3Log", 2];