...
if ((typeOf _building) == _vaultHouse && (nearestObject[_pos, "Land_Dome_Small_F"]) getVariable["locked", true]) exitWith {
    hint localize "STR_ISTR_Bolt_Exploit"
};
...
from 1 to _doors do {
    ...
    if (player distance _worldSpace < 2) exitWith {
        _door = _i;
    };
