#define DISPLAY_NAME CAU_UserInputMenus_displayProgressBar#include "cau\userinputmenus\systems\_macros.inc"#include "cau\userinputmenus\systems\_defines.inc"#define VAR(suffix) QUOTE(JOIN(DISPLAY_NAME, JOIN(_, suffix)))#define SVAR(n, v) _parentDisplay setVariable[VAR(n), v]#define GVAR(n, v)(_parentDisplay getVariable[VAR(n), v])
params[["_parameters", [], [[]]], ["_title", "Progress", [""]], ["_code", {},
[{}]], ["_parentDisplay", displayNull, [displayNull]]];
_parameters params[["_duration", 0, [0]], ["_image", "", [""]], ["_overlay", true, [true]], ["_arguments", []], ["_condition", {},
[{}]], ["_onProgress", {},
[{}]]];
if (_duration <= 0) exitWith {
    ["Duration too short: %1", _duration] call BIS_fnc_error
};
...
if (isNull _display) exitWith {
    ["Unable to create display on parent: %1", _parentDisplay] call BIS_fnc_error
};
...
if _overlay then {
    ...
    _ctrlButton ctrlAddEventHandler["ButtonClick", {
        private _parentDisplay = ctrlParent(_this #0);1 call GVAR(exit,{});
}];};_parentDisplay displayAddEventHandler["Unload",{params["_parentDisplay","_code"];if(_code= =2) then {
            1 call GVAR(exit, {})
        };
    }];
    private _exit = {
        ...
        [_this == 0, _code] call {
            ...
            params["_completed"];
            [] call(_this #1);};};SVAR(code,_code);SVAR(arguments,_arguments);SVAR(condition,_condition);SVAR(onProgress,_onProgress);SVAR(exit,_exit);SVAR(title,_title);SVAR(duration,_duration);SVAR(startTick,diag_tickTime);SVAR(overlay,_overlay);private _eachFrameID= addMissionEventHandler["EachFrame", {
                disableSerialization;
                USE_DISPLAY(THIS_DISPLAY);
                private _parentDisplay = ctrlParent _display;
                ...
                if (!isNil "_canProgress" && { ! (_canProgress isEqualType true) || { ! _canProgress
                    }
                }) exitWith {
                    1 call GVAR(exit, {})
                };
