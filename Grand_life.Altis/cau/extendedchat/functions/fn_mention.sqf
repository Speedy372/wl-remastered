#define THIS_FUNC FUNC(mention)#include "cau\extendedchat\functions\_macros.inc"#include "cau\extendedchat\functions\_defines.inc"
SWITCH_SYS_PARAMS;
switch _mode do {
case "parse":
    {
                ...
                case "p":
                    {
                        ...
                        if (count _messageMentionIDChars == 0) then {
                            {
                                ...
                                if (_unitID isEqualTo _messageMentionID) exitWith {
                                    if (_x isEqualTo player) then {
                                        _messageMentionsSelf = true
                                    };
                                    private _unitName = "@" + (["StreamSafeName", [getPlayerUID _x, UNIT_NAME(_x)]] call FUNC(commonTask));
                                    _params set[_fei, text _unitName setAttributes["color", (["get", VAL_SETTINGS_KEY_TEXT_MENTION_COLOR] call FUNC(settings)) call BIS_fnc_colorRGBAtoHTML]];
                                };
                            }
                            forEach allPlayers;
                        };
                        ...
                        if (count _messageMentionIDChars == 0) then {
                            ...
                            if (!isNull _group) then {
                                ...
                                _params set[_fei, text("@" + groupId _group) setAttributes["color", (["get", VAL_SETTINGS_KEY_TEXT_MENTION_COLOR] call FUNC(settings)) call BIS_fnc_colorRGBAtoHTML]];
                            };
                        };
                    };
                case "r":
                    {
                        ...
                        if (count _messageMentionIDChars == 0) then {
                            ...
                            _params set[_fei, text("@" + _role #1) setAttributes["color",_role# 2]];
                        };
                    };
                };
            };
    ...
    if (_mentionGroupsMode == 2) exitWith {
        _params
    };
                        ...
                        if (count _messageMentionIDChars == 0) then {
                            {
                                ...
                                if (str _unitID isEqualTo _messageMentionID) exitWith {
                                    _mentions pushBack _unitID
                                };
                            }
                            forEach allPlayers;
                        };
                ...
                case "r":
                    {
                        ...
                        if (count _messageMentionIDChars == 0) then {
                            private _role = ["getRole", parseNumber _messageMentionID] call FUNC(role);
                            if ! (_role isEqualTo[]) then {
                                _mentions pushBack _role #3};};};};};}forEach _params;_mentions };};
